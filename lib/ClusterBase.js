...
module.exports = function() {
  function ClusterBase(metaData, coap) {
    ...
    this.commandArgs = function(commandName) {
      var commandData = _.find(this.meta.commands, function(commandData) {
        return commandData.name == commandName;
      });

      if (!commandData) return;

      return _.map(commandData.args, function(argData, argId) {
        return argData;
      });
    };
    ...
    this._defineCommand = _.bind(function(commandId) {
      ...
      this[commandName] = function(args, callback) {
        ...
        var missingArgs = _.difference(_.map(this.commandArgs(commandName), 'name'), _.keys(args));

        if (missingArgs.length) {
          callback('Missing arguments ' + missingArgs.join(', ') + '.');
          return;
        }
    ...
    this._buildCommandPayload = _.bind(function(data, commandData) {
      ...
      Object.getOwnPropertyNames(data).forEach(function(argName) {
        var argId = _.findKey(commandData.args, function(argData, argId, three) {
          return argName == argData.name;
        });
