...
module.exports = function() {
  function ClusterBase(metaData, zclip) {
    this.meta = this._initMetaData(metaData);
    this.coap = zclip.coap;
    this.zclip = zclip;
    this.commands = {};
    this._commandList = [];
  ...
  ClusterBase.prototype.fetchIp = function(callback) {
    if (this.ip) {
    ...
    rd.lookup({ uid: this.uid }, function(err, resources) {
      ...
      // if (!resource) {
      //   callback(`No resources found with uid ${cluster.uid}`);
      // }
  ...
  ClusterBase.prototype.rd = function() {
    if (!this.rdIp)
      return;

    return this.zclip.RD({ ip: this.rdIp, port: this.rdPort });
  }

  ClusterBase.prototype.commandNames = function() {
  ...
  ClusterBase.prototype._defineCommand = function(commandId) {
    ...
    this[commandName] = function(args, callback) {
      ...
      this.fetchIp(function(err, ip) {
        var request = cluster.post(cluster.basePath + 'c/' + commandId);
        var payload = cluster._encodePayload(args, commandData);
