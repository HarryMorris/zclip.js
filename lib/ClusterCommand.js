...
ClusterCommand.prototype.call = function(args, callback) {
  ...
  var request = this.cluster.post('c/' + this.commandId, payload);

  request.on('response', (response) => {
    var responsePayload = this.decodePayload(response.payload);
    callback(null, responsePayload, response.code);
  });
